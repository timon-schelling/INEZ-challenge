...
open class MapQueryValue(
    ...
    operator fun get(key: Any): MapQueryValue {
        if (isMap())
            if (contains(key))
                return MapQueryValue(map[key]!!, MapQueryValueParent(this))
            else
                return placeHolderWithMapDefault(key)
        else if (isList())
            if (key is Number)
                if (contains(key))
                    return MapQueryValue(list[key.toInt()], MapQueryValueParent(this))
                else
                    return placeHolderWithListDefault(key)
    ...
    fun removeFromParent(recursively: Boolean = false) {
        if (hasParent()) {
            if (parent!!.obj.isMutableMap()) {
                ...
                map.entries.filter { it.value === value }
                        .forEach { parent.obj.remove(it.key, recursively) }
            }
            else if (parent.obj.isMutableList()) {
                val list = parent.obj.mlist
                list.filter { it === value }
                        .forEach { parent.obj.remove(list.indexOf(it), recursively) }
            }
        }
    }
    ...
    fun remove(key: Any, recursively: Boolean = false) {
        if (isMutableMap()) {
            ...
            if (key is Number) {
                if (contains(key))
                    mlist.removeAt(key.toInt())
            }
            else throw MapQueryExceptionUseNumericKeyForList()

        if (recursively)
            removeFromParentIfEmpty(recursively)
    }
    ...
    fun contains(key: Any): Boolean {
        if (isMap())
            ...
            if (key is Number)
                return list.containsIndex(key.toInt())
            else throw MapQueryExceptionUseNumericKeyForList()
        return false
    }
